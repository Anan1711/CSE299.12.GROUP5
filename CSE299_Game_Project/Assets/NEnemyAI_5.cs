...
public class NEnemyAI_5 : MonoBehaviour
{
    ...
    void UpdatePath()
    {
        ...
        if (seeker5.IsDone())
        {
            ...
            seeker5.StartPath(rb5.position, target.position, OnPathComplete);
        }
    ...
    void FixedUpdate()
    {
        ...
        if (currentWaypoint5 >= path5.vectorPath.Count)
        {
            reachedEndOfPath5 = true;

            if (reachedEndOfPath5 == true)
            {
                Attack();
            }
            return;
        ...
        else
        {
            ...
            reachedEndOfPath5 = false;

            if (reachedEndOfPath5 == false)
            {
                StopAttack();
            }
        ...
        Vector2 direction5 = ((Vector2)path5.vectorPath[currentWaypoint5] - rb5.position).normalized;
        ...
        Vector2 force5 = direction5 * speed5 * Time.deltaTime;
        ...
        float distance5 = Vector2.Distance(rb5.position, path5.vectorPath[currentWaypoint5]);


        // Checking if we reached that current waypoint
        if (distance5 < nextwaypointDistance5)
        {
        ...
        if (rb5.velocity.x <= -0.01f)
        {
            transform.localScale = new Vector3(-1f, 1f, 1f);
        }
        // if it is positve then we are moving to the right
        else if (rb5.velocity.x >= 0.01f)
        {
            transform.localScale = new Vector3(1f, 1f, 1f);
        }

        animator.SetFloat("Speed", Mathf.Abs(rb5.velocity.x));
