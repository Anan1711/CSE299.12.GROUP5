...
public class NEnemyAI_4 : MonoBehaviour
{
    ...
    void UpdatePath()
    {
        ...
        if (seeker4.IsDone())
        {
            ...
            seeker4.StartPath(rb4.position, target.position, OnPathComplete);
        }
    ...
    void FixedUpdate()
    {
        ...
        if (currentWaypoint4 >= path4.vectorPath.Count)
        {
            reachedEndOfPath4 = true;

            if (reachedEndOfPath4 == true)
            {
                Attack();
            }
            return;
        ...
        else
        {
            ...
            reachedEndOfPath4 = false;

            if (reachedEndOfPath4 == false)
            {
                StopAttack();
            }
        ...
        Vector2 direction4 = ((Vector2)path4.vectorPath[currentWaypoint4] - rb4.position).normalized;
        ...
        Vector2 force4 = direction4 * speed4 * Time.deltaTime;
        ...
        float distance4 = Vector2.Distance(rb4.position, path4.vectorPath[currentWaypoint4]);


        // Checking if we reached that current waypoint
        if (distance4 < nextwaypointDistance4)
        {
        ...
        if (rb4.velocity.x <= -0.01f)
        {
            transform.localScale = new Vector3(-1f, 1f, 1f);
        }
        // if it is positve then we are moving to the right
        else if (rb4.velocity.x >= 0.01f)
        {
            transform.localScale = new Vector3(1f, 1f, 1f);
        }

        animator.SetFloat("Speed", Mathf.Abs(rb4.velocity.x));
